...
def run_agent():
    ...
    try:
        from agent_runner import log_signal
        log_signal(signal, confidence, reason)
    except ImportError:
        pass
    ...
    try:
        ...
        try:
            from agent_runner import log_trade as api_log_trade
            api_log_trade(contract.symbol, qty, str(
                result.status), str(result.id))
        except ImportError:
            pass
        log_trade(
